...
w_auther = 0.45#0.3#0.15
w_conver = 0.5#0.6#0.8
w_temp = 0.05#0.1#0.05
...
def plotThreadNum(realThreadDict, _threadDict):
    ...
    ptitle = 'fval_'+str(int(fvalue(realThreadDict, _threadDict)*10000))+'w_contextfree'+str(int(w_contextFree*100))+'w_conver'+str(int(w_conver*100))+'w_auther'+str(int(w_auther*100))
    plt.title(ptitle)
...
def tuning(w_contextFree=0.14, w_auther=0.3, w_conver=0.6, threshold=0.6):
    """Tuning parameters.
    ...
    for row in ann2.itertuples():
        ...
        msgdate = row[4]
        msgdate = (msgdate - begin_date).days + 1
        _threadDict = similarity(ann2, msgid, msgdate, _threadDict, threshold)

    # Then compute the F value.
    plotThreadNum(realThreadDict, _threadDict)
    return fvalue(realThreadDict, _threadDict)

#TODO
#BASED ON DIFF_REAL_DETECTED_THREAD, FIX SIMILARITY FUNCTION, FIND OUT THE REASON WHY SO MANY TINY THREADS.
'''
通过调节w_contextfree的大小，其实就是通过context的信息对原始的信息进行平滑，如果该值越大，说明更多的采用原始的信息，通常短语句的原始信息差异较大
 ，会导致cluster的数量变多。
如果使用形心呢？不好说。
在现在的图片的基础上，找出最好的，然后调节每个context的权重
现在最好的结果是w_contextfree = 0.14， threshold = 0.6, w_auther=0.3, w_conver=0.6, w_temp=0.1, fvalue = 0.624
回到最原始的问题，thread detection的本质是将讨论同一个topic的msg聚到一起，所以如果有方法提取topic，再将提到类似topic的msg聚到一起不就
完成了thread detection吗？
先有一个用来训练的词库，通过这个词库，我可以知道哪些词经常和哪些词同时出现（或者相邻，或者是词的local network），那这个局部的网络（或者community）就可能是讲同一个topic的
'''
#%% Tuning context weights
# just raise w_conver
tuning(w_conver = 0.9, w_auther = 0.05)
# maybe w_conver should not be such high, cause some @ information is missing.
# w_auther up to 0.5 and w_conver is lower to 0.45
# 为什么tuning的值没有改变，本来是应该变化的？
tuning(w_conver = 0.5, w_auther = 0.45)